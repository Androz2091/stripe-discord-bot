...
const makeMemberExpire = async (customer: DiscordCustomer, member: GuildMember, guild: Guild) => {
    ...
    (guild.channels.cache.get(process.env.LOGS_CHANNEL_ID) as TextChannel).send(`:arrow_lower_right: **${member?.user?.tag || 'Unknown#0000'}** (${customer.discordUserId}, <@${customer.discordUserId}>) has completely lost access. Customer email is \`${customer.email}\`.`);
}
